function output_data=HID_CSMA_DCF(Arrv_rate_A,Arrv_rate_C)
    ...
    while i<sim_slot
        ...
        for node = 1:2
            ...
            elseif(Node_status(node,i) == 1 && bck_of(node) == 0 && back_off_finish(node) == 1 && send_frame(node) == 1)
                channel_s(node,i:i+frame_slot+sifs_slot+ack_slot) = 1;
                back_off_finish(node) = 0;
                NAV(node,i) = frame_slot+sifs_slot+ack_slot;
                NAV(node,i+1) = NAV(node,i) - 1;
                Node_status(node,i+1 : i+frame_slot) = 1;
                send_frame(node) = 0;
            elseif channel_s(node,i) == 1 && NAV(node,i) > sifs_slot+ack_slot
                NAV(node,i+1) = NAV(node,i) - 1;
            ...
            elseif channel_s(node,i) == 1 && (Node_status(3,i) == 1 || Node_status(4,i) == 1) && NAV(node,i) > ack_slot
                NAV(node,i+1) = NAV(node,i) - 1;
            elseif channel_s(node,i) == 1 && (Node_status(3,i) == 1 || Node_status(4,i) == 1) && NAV(node,i) == ack_slot
                if no_ACK(node) == 1
