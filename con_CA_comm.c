    ...
    for (int i = 0; i < sim_slot; i++)
    {
        ...
        for (int len = 0; len < 2; len++)
        {
            ...
            else if (status[len][i] == 1 && channel_s[i] == 0 && bck_of[len] == 1 && flag[len] == 0)
            {
                ...
                if (netf == 1)
                {
                    ...
                    if (flag[0] == 1 && flag[1] == 0) //status to 1 till the acknowledghement is received
                        ...
                        occupy_slot_2dem((int *)status, 1, i, i + rts_slot + sifs_slot + cts_slot + 1, sim_slot, 1); //both are sending and there uis collision
            ...
            else if (status[len][i] == 1 && bck_of[len] == 0 && prev_back_off[len] == 1 && flag[len] == 1)
            {
                ...
                occupy_slot_1dem(channel_s, i, i + rts_slot + sifs_slot + cts_slot, sim_slot, 1);
                if (len == 0) //inidcate for how much duration the of the channnel the channel is taken or occupied
                    OccupyChannel1 = OccupyChannel1 + rts_slot + sifs_slot + cts_slot;
                else
                    OccupyChannel2 = OccupyChannel2 + rts_slot + sifs_slot + cts_slot;
                ...
                NAV[len][i] = rts_slot + sifs_slot + cts_slot;
            ...
            else if (channel_s[i] == 1 && (NAV[len][i] > sifs_slot + cts_slot))
            {

                NAV[len][i_next] = NAV[len][i] - 1;           // decrease tehe NAVC l;emgth
                if (NAV[len][i_next] == sifs_slot + cts_slot) // if transmission is juxt finishing
                    if (status[0][i] == 1 && len == 0)        //inidcate the station is in charge
                    {
                        occupy_slot_2dem((int *)status, 2, i, i + sifs_slot + cts_slot, sim_slot, 1); //time to send acknowledgement now from the node, go to receiver
                        status[0][i_next] = 0;                                                        //node is done with transmission and shouldnt be transmission state
                    }
                    else if (status[1][i] == 1 && len == 1)
                    {
                        occupy_slot_2dem((int *)status, 3, i, i + sifs_slot + cts_slot, sim_slot, 1);
                        status[1][i_next] = 0;
                    }
            }
            else if (channel_s[i] == 1 && (status[2][i] == 1 || status[3][i] == 1) && NAV[len][i] > cts_slot)  //either of the receiver are transmitting and in siffs slot
                NAV[len][i_next] = NAV[len][i] - 1;                                                            // redcue for the siffs slot
            else if (channel_s[i] == 1 && (status[2][i] == 1 || status[3][i] == 1) && NAV[len][i] == cts_slot) //finsihged with sifs
            {
                if (noCts == 1) //no ack...means collision
                {
                ...
                {
                    sendCts = 1; // send ack so that data is successfully transmitted
                    NAV[len][i_next] = NAV[len][i] - 1;
                }
            }
            else if (channel_s[i] == 1 && (status[2][i] == 1 || status[3][i] == 1) && NAV[len][i] == 1) //finsiged with acknowledgement duration
            {
                ...
                {
                    if (sendCts == 1) //data successfully send
                    {
                        // printf("%i, ", i);
                        //frm_rcv_slot1[frm_rcv_num1++] = i;
                        OccupyChannel1 = OccupyChannel1 + sifs_slot + frame_slot + sifs_slot + ack_slot;
                        frm_lst[0][s1] = 1; //indicate that the frame was  successfully transmitetd
                        sendCts = 0;
                        netf = 0;
                        flag[0] = 0;
                        NAV[len][i] = NAV[len][i] - 1;
                ...
                else if (status[3][i] == 1)
                {
                    if (sendCts == 1)
                    {
                        ...
                        OccupyChannel2 = OccupyChannel2 + sifs_slot + frame_slot + sifs_slot + ack_slot;
                        frm_lst[1][s2] = 1;
                        sendCts = 0;
                        netf = 0;
                        flag[1] = 0;
